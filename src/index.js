...
function collectDOMStat(root) {
  ...
  (function DOMStatRecursive(root) {
    for (let i = 0; i < root.childNodes.length; i++) {
      const node = root.childNodes[i];

      switch (node.nodeType) {
        case 1:
          stat.tags[node.tagName] = stat.tags[node.tagName] ? ++stat.tags[node.tagName] : 1;
          stat.classes = getNodeClasses(stat.classes, node);
          DOMStatRecursive(node);
          break;
        case 3:
          stat.texts++;
          break;
  ...
  function getNodeClasses(stat = {}, node) {
    const classList = node.classList;

    for (let i = 0; i < classList.length; i++) {
      const _class = classList[i];

      stat[_class] = stat[_class] ? ++stat[_class] : 1;
    }
...
function observeChildNodes(where, fn) {
  ...
  const observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      actionMutation(mutation.addedNodes, 'insert');
      actionMutation(mutation.removedNodes, 'remove');
    });
  });

  function actionMutation(mutation, action) {
    const nodes = Object.keys(mutation);
